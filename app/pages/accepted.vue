...
const { acceptedResumes } = useResumeStore()
const menuItems = acceptedResumes
      ...
      >
        ...
        >
          ...
          <div class="mt-8 rounded-lg border border-green-200 bg-green-50 p-5">
            <p class="text-xs font-semibold uppercase tracking-wide text-green-700">Acceptance Comment</p>
            <p class="mt-2 text-sm leading-relaxed text-gray-700">
              {{ menuItems.find(item => item.id === selectedItemId)?.acceptanceComment }}
            </p>
          </div>
        </div>
      </div>
